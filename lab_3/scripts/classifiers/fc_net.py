...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            w_key = f'W{i+1}'
            b_key = f'b{i+1}'
            self.params[w_key] = weight_scale * np.random.randn(all_dims[i], all_dims[i+1])
            self.params[b_key] = np.zeros(all_dims[i+1])
            
            if self.normalization == 'batchnorm' and i < self.num_layers - 1:
                self.params[f'gamma{i+1}'] = np.ones(all_dims[i+1])
                self.params[f'beta{i+1}'] = np.zeros(all_dims[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            out, fc_cache = affine_forward(current_input, self.params[f'W{i}'], self.params[f'b{i}'])
        ...
        w_last, b_last = f'W{self.num_layers}', f'b{self.num_layers}'
        scores, caches[self.num_layers] = affine_forward(current_input, 
                                                         self.params[w_last], 
                                                         self.params[b_last])
        ...
        L = self.num_layers
        dout, dWL, dbL = affine_backward(dscores, caches[L])
