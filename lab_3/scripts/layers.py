...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_hat, gamma, x_centered, std, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x_hat, gamma, x_centered, std, eps = cache
    N, D = dout.shape
    ...
    dx_centered_part1 = dx_hat / std
    
    dstd = -dinv_std / (std**2)
    dvar = 0.5 * dstd / std
    
    dx_centered_part2 = (2.0 / N) * x_centered * dvar
    dx_centered = dx_centered_part1 + dx_centered_part2
    
    dmean = -np.sum(dx_centered, axis=0)
    dx = dx_centered + (1.0 / N) * dmean
